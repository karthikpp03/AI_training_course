...
img_h, img_w = 32, 320  # same as in model
batch_size = 16
epochs = 50
save_every = 5
...
# We do resizing + some augmentation
transform = transforms.Compose([
    transforms.Resize((img_h, img_w)),
    transforms.ColorJitter(brightness=0.3, contrast=0.3),  # random color changes
    transforms.RandomAffine(degrees=5, translate=(0.1, 0.1), scale=(0.9, 1.1)),  # small rotations/moves
    transforms.ToTensor(),
...
class LabelSmoothingCrossEntropy(nn.Module):
    """
    Instead of normal cross-entropy, this smooths targets a bit.
...
def beam_search_decode(model, image, beam_width=5, max_len=120):
    """Try multiple sequences and pick the best one"""
    model.eval()
    device = next(model.parameters()).device
    with torch.no_grad():
        ...
        tokens = [vocab[idx] for idx in best_sequence[1:] if idx != pad_idx and idx != eos_idx]
        return ''.join(tokens)

def calculate_cer(predicted, ground_truth):
    """Character Error Rate"""
    from difflib import SequenceMatcher
    return 1 - SequenceMatcher(None, ground_truth, predicted).ratio()
...
def evaluate_model(model, test_loader, epoch, use_beam_search=False):
    ...
    with torch.no_grad():
        for imgs, targets, raw_text in test_loader:
            ...
            if total_samples <= 3:
                print(f"  Example {total_samples}: GT='{gt}', Pred='{pred}', CER={calculate_cer(pred, gt):.3f}")

    char_acc = (correct_chars / total_chars) * 100 if total_chars > 0 else 0
    avg_cer = total_cer / total_samples
    avg_word_acc = (total_word_acc / total_samples) * 100
    print(f" -> Epoch {epoch}: Char Acc={char_acc:.2f}%, CER={avg_cer:.3f}, Word Acc={avg_word_acc:.2f}%")
...
for epoch in range(1, epochs + 1):
    model.train()
    running_loss = 0.0
    current_tf_ratio = max(0.3, 0.9 - (epoch / epochs) * 0.6)  # scheduled teacher forcing

    for imgs, targets, _ in train_loader:
        imgs, targets = imgs.to(device), targets.to(device)
        optimizer.zero_grad()
        outputs = model(imgs, targets=targets, teacher_forcing_ratio=current_tf_ratio,
                        sos_idx=sos_idx, eos_idx=eos_idx)
        batch_size, T, V = outputs.size()
        outputs_flat = outputs.view(-1, V)
        target_flat = targets[:, :T].contiguous().view(-1)
    ...
    print(f"Epoch {epoch}/{epochs} - Loss: {avg_loss:.4f}, TF: {current_tf_ratio:.2f}, LR: {scheduler.get_last_lr()[0]:.6f}")
